...
TEST(VectorTest, Magnitude) {
  Vector<int> v(Point<int>(2, 3, 6));
  EXPECT_DOUBLE_EQ(v.magnitude(), std::sqrt(49.0));
}

TEST(VectorTest, DotProduct) {
  Vector<int> v1(Point<int>(1, 2, 3));
  Vector<int> v2(Point<int>(4, -5, 6));
  int result = v1.dot_product(v2);
  EXPECT_EQ(result, 12);
}

TEST(VectorTest, CrossProduct) {
  Vector<int> v1(Point<int>(1, 0, 0));
  Vector<int> v2(Point<int>(0, 1, 0));
  Vector<int> cross = v1.cross_product(v2);
  Vector<int> expected(Point<int>(0, 0, 1));
...
TEST(VectorTest, Addition) {
  ...
  Vector<int> expected(Point<int>(5, 7, 9));
  EXPECT_EQ(sum, expected);
}

TEST(VectorTest, Equality) {
  Vector<int> v1(Point<int>(1, 2, 3));
  Vector<int> v2(Point<int>(1, 2, 3));
  Vector<int> v3(Point<int>(3, 2, 1));
  EXPECT_TRUE(v1 == v2);
  EXPECT_FALSE(v1 == v3);
}

TEST(VectorTest, ScalarMultiplication) {
  Vector<int> v(Point<int>(1, 2, 3));
  Vector<int> scaled1 = v * 2;
  Vector<int> scaled2 = 2 * v;
  Vector<int> expected(Point<int>(2, 4, 6));
